...
class CustomBootBot extends BootBot {
  ...
  async handleResponseMessage(metadata, message) {
    ...
    if(message?.path){
      await this.sendAttachment(userId, message.type, message.path)
    }
    if (!message.elements) {
      return null;
    }
...
function init(app, bot) {
  ...
  const greetingMessage = process.env.SET_GREETING_MESSAGE_FACEBOOK || 'Warm welcome to Palm Mind Technology I am Palm Bot- Your Virtual Assistant. I am here to help you with your queries related to our products and services.'
