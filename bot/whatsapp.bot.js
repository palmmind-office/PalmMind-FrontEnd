...
class WhatsappBot extends EventEmitter {
    ...
    async handleResponseMessage(metadata, message) {
        ...
        if (custom.hasOwnProperty('text') && !custom.path || custom.hasOwnProperty('message')) {
        ...
        switch (custom.type) {
            case 'image':
                ...
                break;
            case 'audio':
                let audiodata = {link: custom.path};
                await this.sendText(userId, custom.text)
                await this.sendMedia(userId, 'audio', audiodata)
            break;
                case "media":
                    const { fileType, url, caption: fileCaption } = custom;
                    const type = fileType === "file" ? "document" : fileType;
                    const data = { link: url, ...(["image", "video"].includes(type) ? { caption: fileCaption } : {}) };
                    if (type === "document") {
                      data.filename = url.split("/").at(-1).split("-").at(-1);
                    }
                    if (!data.caption && fileCaption) {
                      await this.sendText(userId, fileCaption);
                    }
